    ...
    <div className="bg-background">
      <section className="section">
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[1.4fr,1fr]">
          <img src={project.img} alt={project.title} className="w-full h-80 object-cover rounded-xl border" />
          <div className="rounded-xl border bg-white p-6 shadow-sm">
            ...
            <ul className="mt-4 list-disc pl-5 text-sm text-foreground/70 space-y-1">
              ...
              <li>Outcomes: Uptime improvement, safety alignment, energy savings</li>
            </ul>
          </div>
        </div>
            ...
            <Dialog key={i}>
              <DialogTrigger asChild>
                <button className="group rounded-lg overflow-hidden border bg-white hover:shadow">
                  <img src={g} alt={`${project.title} ${i+1}`} className="h-36 w-full object-cover transition group-hover:scale-[1.03]" />
                </button>
              </DialogTrigger>
              <DialogContent className="sm:max-w-3xl">
                ...
                <img src={g} alt={`${project.title} ${i+1}`} className="w-full rounded-md object-cover" />
              </DialogContent>
            </Dialog>
          ))}
        </div>
        ...
        <div className="mt-10 grid gap-6 md:grid-cols-3">
          {[
            ["Objectives", "Reliability, safety, and cost efficiency with minimal downtime."],
            ["Challenges", "Tight timelines, complex interfaces and compliance constraints."],
